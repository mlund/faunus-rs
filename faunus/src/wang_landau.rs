...
/// A validated Wang-Landau run, ready to spawn its walkers.
///
/// Building one parses the config, builds the flat-histogram grid, constructs the shared base
/// context (Hamiltonian) and checks that both collective variables resolve against it. A
/// `--check` dry run can stop at [`prepare`](Self::prepare) with no walker threads spawned and
/// no state directory created. [`run`](Self::run) then drives the walkers to convergence.
pub(crate) struct WangLandau {
    input: PathBuf,
...
impl WangLandau {
    /// Validate the input and build the shared context and grid without spawning walkers or
    ...
    /// Drive the walkers to convergence and write the free-energy surface to `output`.
