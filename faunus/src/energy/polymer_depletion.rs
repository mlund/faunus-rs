...
impl PolymerDepletion {
    ...
    pub(super) fn to_yaml(&self) -> yaml_serde::Value {
        ...
        let molecules: Vec<yaml_serde::Value> = self
            .colloid_molecule_names
            .iter()
            .cloned()
            .map(Into::into)
            .collect();
        map.insert("molecules".into(), yaml_serde::Value::Sequence(molecules));
        ...
        if let Some(steric) = &self.steric {
            let mut steric_map = yaml_serde::Mapping::new();
            steric_map.insert("epsilon0_prime".into(), steric.config.epsilon0_prime.into());
            steric_map.insert("g0".into(), steric.config.g0.into());
            let gs_vals: Vec<yaml_serde::Value> = steric.g_s.iter().map(|&v| v.into()).collect();
            steric_map.insert("g_s".into(), yaml_serde::Value::Sequence(gs_vals));
            let ht_vals: Vec<yaml_serde::Value> =
                steric.h_tilde_eff.iter().map(|&v| v.into()).collect();
            steric_map.insert("h_tilde_eff".into(), yaml_serde::Value::Sequence(ht_vals));
            map.insert(
                "steric_adsorption".into(),
                yaml_serde::Value::Mapping(steric_map),
            );
        }

        let colloids: Vec<yaml_serde::Value> = self
            .colloids
            .iter()
            .map(|c| {
            ...
            .collect();
        map.insert("colloids".into(), yaml_serde::Value::Sequence(colloids));
...
colloid_radius: 5.0
"#;
        let builder: PolymerDepletionBuilder = yaml_serde::from_str(yaml).unwrap();
...
molecules: [Colloid]
"#;
        let builder2: PolymerDepletionBuilder = yaml_serde::from_str(yaml2).unwrap();
