...
rigid molecules and stores the pairwise energy on an icosphere mesh with
barycentric interpolation for the angular dimensions.

At runtime, the current separation and orientations of each molecule pair
are mapped to 6D table coordinates and the energy is obtained by interpolation.
